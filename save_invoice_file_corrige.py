"""
FONCTION CORRIGÉE POUR LA SAISIE DES CODES DANS LES FICHIERS EXCEL DE FACTURATION

Corrections apportées:
1. Vérification exacte du numéro de facture (plus de correspondance partielle)
    ...
    try:
        ...
        try:
            ...
            for row_data in validated_rows:
                ...
                # Nettoyer le numéro de facture (enlever "Facture N°" s'il est présent)
                if "facture n°" in facture_num.lower():
                    facture_num_clean = facture_num.lower().replace("facture n°", "").strip()
                elif "facture n" in facture_num.lower():
                    facture_num_clean = facture_num.lower().replace("facture n", "").strip()
                elif "facture" in facture_num.lower():
                    facture_num_clean = facture_num.lower().replace("facture", "").strip()
                else:
                    facture_num_clean = facture_num.lower().strip()
                
                # Log pour débogage
                logger.info(f"Traitement de la facture {facture_num} (UH: {uh}, numéro nettoyé: {facture_num_clean})")
                            ...
                            for col in range(1, 20):  # Limiter aux 20 premières colonnes
                                try:
                                    ...
                                    if cell_value is not None:  # Vérifier que la cellule n'est pas vide
                                        ...
                                        # Chercher le numéro de facture exact
                                        if "facture n°" in cell_text:
                                            cell_num = cell_text.replace("facture n°", "").strip()
                                            if cell_num == facture_num_clean:
                                            ...
                                            cell_num = cell_text.replace("facture", "").strip()
                                            if cell_num == facture_num_clean:
                                                facture_trouvee = True
                                                facture_row = row
                                                facture_col = col
                                                logger.info(f"Numéro de facture {facture_num} trouvé dans la cellule ({row}, {col}): '{cell_text}'")
                                                break
                                        elif cell_text == facture_num_clean:
                                            facture_trouvee = True
                                            facture_row = row
                                            facture_col = col
                                            logger.info(f"Numéro de facture {facture_num} trouvé dans la cellule ({row}, {col}): '{cell_text}'")
