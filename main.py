...
from PyQt5.QtWidgets import (
    ...
    QSpinBox, QStyle, QStyleFactory, QFormLayout, QGridLayout, QSplitter, QDialog, 
...
class DatabaseLoaderThread(QThread):
    ...
    def __init__(self, db_file):
        super().__init__()
        # Flag d\'initialisation de l\'interface de recherche
...
def normalize_text(text):
    ...
    if text is None:
        return ""
    # Convertir en chaîne, supprimer les accents, mettre en minuscules et supprimer les espaces superflus
...
class Database:
    ...
    def search_entries(self, query, category=None, exact_match=False):
        ...
        for name, entry in self.data.items():
            # Si une catégorie est spécifiée, ne chercher que dans cette catégorie
            if category and category in entry:
                field_value = normalize_text(entry[category])
                if exact_match:
                ...
                for value in entry.values():
                    if not value:
                        continue
                    field_value = normalize_text(value)
                    if exact_match:
                        if field_value == query:
                            found = True
                            break
...
class MainWindow(QMainWindow):
    ...
    def setup_toolbar(self):
        ...
        # Créer la barre d'outils avec un nom d'objet
        self.toolbar = QToolBar("Outils")
        self.toolbar.setObjectName("mainToolBar")  # Définir un nom d'objet explicite
        self.addToolBar(self.toolbar)
        
        # Ajouter des actions à la barre d'outils
        open_action = QAction("Ouvrir un fichier Excel", self)
        ...
        # Bouton pour rafraîchir la base de données
    ...
    def filter_database(self, force=False):
        """Filtre la base de données en fonction de la recherche
        
        Args:
            force (bool): Si True, force le filtrage même si l'interface n'est pas complètement initialisée
        """
        # Vérifier si l'interface de recherche est complètement initialisée
        # Vérifier si l'interface est prête
        if not hasattr(self, 'db_search_edit') or not hasattr(self, 'db_table'):
            if not force:
                logger.debug("Interface de recherche non encore initialisée")
            return
            
        if self.db_search_edit is None or self.db_table is None:
            if not force:
                logger.debug("Composants de recherche non initialisés")
            return
            
        try:
            search_text = self.db_search_edit.text().strip().lower()
            if not force and not search_text:
            ...
            logger.debug(f"Recherche dans la base de données: '{search_text}'")
            
            # Vérifier si le tableau est initialisé
            if not hasattr(self, 'db_table') or self.db_table is None:
                logger.error("Le tableau de la base de données n'est pas initialisé")
                return
            
            # Vérifier si la base de données est chargée
            if not hasattr(self, 'database') or not self.database._loaded:
                logger.info("Chargement de la base de données...")
                self.database.ensure_loaded(lambda: self.filter_database())
                return
            
            # Désactiver les mises à jour de l'interface pour améliorer les performances
            self.db_table.setUpdatesEnabled(False)
            
            # Si le champ de recherche est vide, afficher toutes les lignes
            if not search_text:
                logger.debug("Champ de recherche vide, affichage de toutes les lignes")
                for row in range(self.db_table.rowCount()):
                    self.db_table.setRowHidden(row, False)
                return
            ...
            for row in range(self.db_table.rowCount()):
                ...
                # Vérifier chaque colonne de la ligne
                ...
                # Masquer ou afficher la ligne selon les résultats
                self.db_table.setRowHidden(row, hide_row)
            
            logger.debug(f"{matches} correspondance(s) trouvée(s) pour '{search_text}'")
            
        except Exception as e:
            logger.error(f"Erreur lors du filtrage de la base de données: {e}", exc_info=True)
            # En cas d'erreur, afficher toutes les lignes
    ...
    def setup_database_interface(self):
        ...
        logger.debug("[SETUP_DB_INTERFACE] Début de l'initialisation de l'interface")
        
        try:
            # Barre de recherche
            search_layout = QHBoxLayout()
            logger.debug("[SETUP_DB_INTERFACE] Création du layout de recherche")
            
            self.db_search_edit = QLineEdit()
            logger.debug(f"[SETUP_DB_INTERFACE] Champ de recherche créé: {self.db_search_edit}")
            
            self.db_search_edit.setPlaceholderText("Rechercher dans la base de données...")
            search_layout.addWidget(self.db_search_edit)
            logger.debug("[SETUP_DB_INTERFACE] Champ de recherche ajouté au layout")
            ...
            logger.debug("[SETUP_DB_INTERFACE] Bouton d'ajout configuré")
            
            self.database_layout.addLayout(search_layout)
            logger.debug("[SETUP_DB_INTERFACE] Layout de recherche ajouté à l'interface")
            
            # Tableau de la base de données
            self.db_table = QTableWidget()
            logger.debug(f"[SETUP_DB_INTERFACE] Tableau créé: {self.db_table}")
            ...
            self.db_table.setColumnWidth(0, 50)
            ...
            logger.debug("[SETUP_DB_INTERFACE] Tableau ajouté au layout")
            
            # Connecter les signaux après l'initialisation complète
            logger.debug("[SETUP_DB_INTERFACE] Tentative de connexion des signaux...")
            self.connect_search_signals()
        # Marquer l\'interface de recherche comme prête
            
        except Exception as e:
            logger.error(f"[SETUP_DB_INTERFACE] Erreur critique: {e}", exc_info=True)
            raise
            
        logger.debug("[SETUP_DB_INTERFACE] Initialisation terminée avec succès")
            
    def connect_search_signals(self):
        """Connecte les signaux pour la fonctionnalité de recherche"""
        logger.debug("[CONNECT_SEARCH_SIGNALS] Début de la connexion des signaux")
        
        # Vérifier que les composants nécessaires existent
        if not hasattr(self, 'db_search_edit') or self.db_search_edit is None:
            logger.error("[CONNECT_SEARCH_SIGNALS] ERREUR: db_search_edit n'est pas initialisé")
            return False
            
        if not hasattr(self, 'db_table') or self.db_table is None:
            logger.error("[CONNECT_SEARCH_SIGNALS] ERREUR: db_table n'est pas initialisé")
            return False
            
        try:
            logger.debug("[CONNECT_SEARCH_SIGNALS] Composants nécessaires trouvés")
            
            # Déconnecter d'abord pour éviter les connexions multiples
            try:
                self.db_search_edit.textChanged.disconnect()
                logger.debug("[CONNECT_SEARCH_SIGNALS] Anciennes connexions déconnectées")
            except (TypeError, RuntimeError) as e:
                # Pas de connexion existante ou erreur de déconnexion
                logger.debug(f"[CONNECT_SEARCH_SIGNALS] Aucune connexion à déconnecter: {e}")
            
            # Créer une fonction wrapper pour gérer les erreurs
            def search_text_changed():
                try:
                    logger.debug("[SEARCH_TEXT_CHANGED] Changement détecté dans le champ de recherche")
                    self.filter_database(force=True)
                except Exception as e:
                    logger.error(f"[SEARCH_TEXT_CHANGED] Erreur lors du filtrage: {e}", exc_info=True)
            
            # Reconnecter avec le wrapper
            self.db_search_edit.textChanged.connect(search_text_changed)
            logger.debug("[CONNECT_SEARCH_SIGNALS] Signal textChanged connecté avec succès")
            
            # Forcer une mise à jour initiale après un court délai
            QTimer.singleShot(500, lambda: self.safe_initial_filter())
            
            return True
            
        except Exception as e:
            logger.error(f"[CONNECT_SEARCH_SIGNALS] ERREUR critique: {e}", exc_info=True)
            return False
    
    def safe_initial_filter(self):
        """Effectue un filtrage initial en toute sécurité"""
        try:
            if hasattr(self, 'db_search_edit') and self.db_search_edit is not None:
                logger.debug("[SAFE_INITIAL_FILTER] Exécution du filtrage initial")
                self.filter_database(force=True)
            else:
                logger.warning("[SAFE_INITIAL_FILTER] Impossible d'exécuter le filtrage initial: db_search_edit non disponible")
        except Exception as e:
            logger.error(f"[SAFE_INITIAL_FILTER] Erreur lors du filtrage initial: {e}", exc_info=True)
    
    def on_db_cell_changed(self, item):
    ...
    def load_invoice_file(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getOpenFileName(
                ...
                "Ouvrir une facture",
                "",
                "Tous les fichiers (*);;Fichiers Excel (*.xlsx *.xls)",
                "Tous les fichiers (*)"
            )
    ...
    def process_invoice_file(self, file_path):
        ...
        try:
            ...
            # Initialiser la liste des factures et le compteur de factures ignorées
            self.invoices = []
            ignored_invoices = 0
                    ...
                    for col_idx in range(1, sheet.max_column + 1):
                        ...
                        if cell.value and "Intitulé" in str(cell.value):
                            try:
                                ...
                                # Vérifier si cette facture a déjà un code client
                                # Le code client est à 2 cellules en dessous et 6 cellules à gauche de la cellule du numéro de facture
                                code_client_row = row_idx + 2  # 2 cellules en dessous
                                code_client_col = col_idx + 7 - 6  # 6 cellules à gauche de la cellule du numéro de facture (qui est à col_idx + 7)
                                code_client_cell = sheet.cell(row=code_client_row, column=code_client_col)
                                has_client_code = code_client_cell.value is not None and str(code_client_cell.value).strip() != ""
                                
                                # Si la facture a déjà un code client, on l'ignore
                                if has_client_code:
                                    ignored_invoices += 1
                                    logger.debug(f"Facture ignorée car elle a déjà un code client: {invoice_number}")
                                    continue
            ...
            # Afficher un message de succès avec le nombre de factures ignorées
            message = f"{len(self.invoices)} factures ont été importées avec succès."
            if ignored_invoices > 0:
                message += f"\n{ignored_invoices} factures ont été ignorées car elles ont déjà un code client."
    ...
    def validate_invoice_row_safe(self, visual_row):
        ...
        try:
            ...
            # Utiliser la méthode validate_invoice_row qui contient déjà la logique pour remplir
            # les informations associées (code client, adresse, code chorus, nom d'établissement)
            # à partir de la base de données lorsqu'un numéro est saisi dans la colonne "Ligne BDD"
            self.validate_invoice_row(visual_row)
            
            logger.info(f"Facture ligne {visual_row} validée avec succès via validate_invoice_row_safe")
    ...
    def process_invoices_with_progress(self, validated_rows, excel_file_path):
        """Traite les factures validées avec une barre de progression"""
        try:
            # Créer une boîte de dialogue de progression
            progress_dialog = QProgressDialog("Saisie des codes en cours...", None, 0, 100, self)
            progress_dialog.setWindowTitle("Saisie des codes")
            progress_dialog.setWindowModality(Qt.WindowModality.WindowModal)
            progress_dialog.setMinimumDuration(0)  # Afficher immédiatement
            progress_dialog.setCancelButton(None)  # Pas de bouton d'annulation
            progress_dialog.setMinimumWidth(300)  # Largeur minimale
            progress_dialog.show()
            QApplication.processEvents()
            
            # Étape 1: Préparation du fichier (10%)
            progress_dialog.setValue(10)
            progress_dialog.setLabelText("Préparation du fichier Excel...")
            QApplication.processEvents()
            ...
            shutil.copy2(excel_file_path, new_file_path)
            logger.info(f"Fichier copié avec succès: {new_file_path}")
            
            # Étape 2: Ouverture du fichier (20%)
            progress_dialog.setValue(20)
            progress_dialog.setLabelText("Ouverture du fichier Excel...")
            QApplication.processEvents()
            
            # Utiliser win32com pour manipuler Excel
            ...
            # Étape 3: Traitement des factures (30-90%)
            for i, row_data in enumerate(validated_rows):
                # Calculer la progression (de 30% à 90%)
                progress_percent = 30 + int(60 * (i / total_rows)) if total_rows > 0 else 90
                progress_dialog.setValue(progress_percent)
                progress_dialog.setLabelText(f"Traitement de la facture {i+1}/{total_rows}...")
                QApplication.processEvents()
                ...
                # Traiter la facture (code existant)
                facture_traitee = False
                feuille_uh_trouvee = False
                
                # Chercher une feuille qui correspond à l'UH
                for j in range(1, workbook.Sheets.Count + 1):
                    ...
                    if uh.lower() in sheet_name.lower():
                        ...
                        # Chercher le numéro de facture dans cette feuille
                        facture_trouvee = False
                        facture_row = None
                        facture_col = None
                        
                        # Rechercher dans les cellules de la feuille
                            ...
                            for col in range(1, sheet.UsedRange.Columns.Count + 1):
                                try:
                                    ...
                                    if cell_value is not None:
                                        ...
                                        # Vérifier si la cellule contient le numéro de facture
                                        if self.est_facture_correspondante(cell_text, facture_num):
            ...
            progress_dialog.setLabelText("Sauvegarde du fichier...")
            QApplication.processEvents()
            ...
            progress_dialog.setLabelText("Traitement terminé avec succès!")
            QApplication.processEvents()
            
            # Fermer la boîte de dialogue après un court délai
            QTimer.singleShot(800, progress_dialog.close)
            
            return new_file_path, factures_traitees
            ...
            logger.error(f"Erreur lors du traitement des factures: {str(e)}", exc_info=True)
            raise Exception(f"Erreur lors du traitement des factures: {str(e)}")
        ...
        try:
            ...
            if not hasattr(self, 'current_excel_file') or not self.current_excel_file:
                ...
                if hasattr(self, 'current_invoice_path') and self.current_invoice_path and os.path.exists(self.current_invoice_path):
                    ...
                    # Ouvrir directement une boîte de dialogue pour sélectionner un fichier Excel
                    file_dialog = QFileDialog()
                    file_dialog.setWindowTitle("Sélectionner un fichier Excel de facturation")
                    file_dialog.setFileMode(QFileDialog.ExistingFile)
                    file_dialog.setNameFilter("Fichiers Excel (*.xlsx *.xls *.xlsm)")
            ...
            for row in range(self.invoice_table.rowCount()):
                ...
                if item and item.background().color().name().lower() == "#0078d7":
            ...
            if not validated_rows:
                QMessageBox.warning(self, "Attention", "Aucune ligne validée (en bleu #0078d7) n'a été trouvée.\nVérifiez que vous avez bien validé des lignes avec le bouton Valider.")
                return False
            
            try:
                # Utiliser la nouvelle méthode avec barre de progression
                new_file_path, factures_traitees = self.process_invoices_with_progress(
                    validated_rows, 
                ...
                # En cas d'erreur, afficher un message et logger l'erreur
                QMessageBox.critical(
                    self,
    ...
    def setup_invoice_interface(self):
        ...
        self.color_filter_combo.addItem("Pas de concordance (Sans couleur)")
    ...
    def save_invoice_as(self):
        ...
        try:
            ...
            # Créer une boîte de dialogue de progression améliorée
            progress_dialog = QProgressDialog("Initialisation de la saisie des codes...", "Annuler", 0, self.invoice_table.rowCount(), self)
            progress_dialog.setWindowTitle("Saisie des codes en cours")
            progress_dialog.setWindowModality(Qt.WindowModality.WindowModal)
            progress_dialog.setMinimumDuration(0)
            progress_dialog.setMinimumWidth(400)  # Largeur minimale pour une meilleure lisibilité
            progress_dialog.setAutoReset(False)   # Empêcher la réinitialisation automatique
            progress_dialog.setAutoClose(False)   # Empêcher la fermeture automatique
            
            # Personnaliser l'apparence de la barre de progression
            progress_bar = progress_dialog.findChild(QProgressBar)
            if progress_bar:
                progress_bar.setTextVisible(True)  # Afficher le pourcentage
                progress_bar.setFormat("%p% - %v/%m factures traitées")
                progress_bar.setStyleSheet(f"QProgressBar {{ text-align: center; }}") 
    ...
    def setup_invoice_tab(self):
        ...
        # Barre de recherche
        search_layout = QHBoxLayout()
        self.invoice_db_search_edit = QLineEdit()
        self.invoice_db_search_edit.setPlaceholderText("Rechercher dans les factures...")
        self.invoice_db_search_edit.textChanged.connect(self.filter_invoices)
        
        search_layout.addWidget(self.invoice_db_search_edit)
    ...
    def setup_database_tab(self):
        ...
        buttons = [
            ...
            ("Exporter JSON", self.export_database_to_excel, "export_db_btn"),
            ("Actualiser", self.refresh_application, "refresh_btn"),
    ...
    def refresh_application(self):
        """Actualise l'ensemble de l'application pour synchroniser les données"""
        try:
            # Afficher un indicateur de chargement
            self.status_indicator.setText("Actualisation en cours...")
            ...
            logger.info("Base de données rechargée lors de l'actualisation")
            
            # Mettre à jour l'affichage de la base de données
            self.update_database_view()
            
            # Mettre à jour l'affichage des factures si nécessaire
            if hasattr(self, 'invoice_table') and self.invoice_table.rowCount() > 0:
                # Sauvegarder les modifications actuelles
                self.process_invoice_matches()
                logger.info("Tableau des factures mis à jour lors de l'actualisation")
            
            # Notification de succès
            self.loading_indicator.stop()
            self.status_indicator.setText("Prêt")
            self.statusBar().showMessage("Actualisation terminée avec succès", 3000)
            
        except Exception as e:
            # Afficher un message d'erreur en cas de problème
            self.loading_indicator.stop()
            self.status_indicator.setText("Erreur")
            error_msg = f"Erreur lors de l'actualisation: {str(e)}"
            logger.error(error_msg)
            QMessageBox.critical(self, "Erreur d'actualisation", error_msg)
            self.statusBar().showMessage("Erreur lors de l'actualisation", 3000)
        ...
        for row in range(self.invoice_table.rowCount()):
            ...
            if selected_filter != "Toutes les couleurs":
                ...
                if item:
                    ...
                    elif selected_filter == "Pas de concordance (Sans couleur)":
                    ...
                    color_match = (selected_filter == "Pas de concordance (Sans couleur)")
        ...
        if col == 3:  # Ligne BDD
            try:
                if value.strip() != "":
                    ...
                    ligne_num = int(value)
                    item.setText(str(ligne_num))  # Formater correctement le nombre
                    self.update_database_line(row, ligne_num)
                    logger.info(f"Ligne BDD mise à jour pour la ligne {row}: {ligne_num}")
                else:
                    # Si la valeur est vide, mettre à null la ligne BDD
                    self.update_database_line(row, None)
                    logger.info(f"Ligne BDD effacée pour la ligne {row}")
            except ValueError:
                ...
                item.setText("")  # Réinitialiser la valeur
    finally:
        self._updating_invoice_item = False
        ...
        for table_name, entries in self.current_tables:
            for entry in entries:
                if current_row == row:
                    ...
                    # Actualiser l'application pour s'assurer que les modifications sont prises en compte
                    if int_value > 0:
                        # Utiliser refresh_application pour une mise à jour complète
                        self.refresh_application()
                        logger.info(f"Application actualisée après mise à jour de la ligne BDD: {int_value}")
                    return
                current_row += 1
...
if __name__ == '__main__':
    ...
    # Ajouter dynamiquement la méthode update_database_from_table à l'instance
    def update_database_from_table(self):
        """Met à jour la base de données à partir du tableau"""
        try:
            data = {}
            for row in range(self.db_table.rowCount()):
                # Récupérer le nom (clé du dictionnaire) - Ignorer la colonne # (index 0)
                nom_item = self.db_table.item(row, 1)  # La colonne 1 contient le nom (après la colonne #)
    ...
    import types
    window.update_database_from_table = types.MethodType(update_database_from_table, window)
